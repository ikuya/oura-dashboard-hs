...
postLoginR = do
    ...
    let pw = case body of
            A.Object o -> case lookup "password" o of
                Just (A.String s) -> s
                _                 -> ""
            _ -> ""
    ok <- checkPassword pw
...
getMetricsR = do
    ...
    returnJson $ A.Object (mapFromList pairs)
...
syncResultToJson r = A.object
    [ "synced" A..= A.Object (mapFromList
        [ (m, A.toJSON c) | (m, c) <- M.toList (Sync.syncedCounts r) ])
    , "errors" A..= A.Object (mapFromList
        [ (m, A.toJSON e) | (m, e) <- M.toList (Sync.syncErrors r) ])
    ]
