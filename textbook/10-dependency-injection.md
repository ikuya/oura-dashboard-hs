...
← [第9章 再帰・畳み込み・遅延評価](09-recursion-folds-laziness.md) | [目次](README.md) | [第11章 aeson で緩い JSON を扱う](11-aeson.md) →
...
- [10.1 インターフェースはただのレコード](#101-インターフェースはただのレコード)
  - [文法メモ: 高階関数](#文法メモ-高階関数)
- [10.2 本番の実装 — 部分適用とクロージャ](#102-本番の実装-部分適用とクロージャ)
- [10.3 テストではレコードを差し替える](#103-テストではレコードを差し替える)
  - [レコード更新構文で 1 フィールドだけ差し替える](#レコード更新構文で-1-フィールドだけ差し替える)
- [10.4 テスト用の差し込み口（test seam）](#104-テスト用の差し込み口test-seam)
- [10.5 依存が関数 1 個なら、レコードすら要らない](#105-依存が関数-1-個ならレコードすら要らない)
- [10.6 型クラスによる DI との比較](#106-型クラスによる-di-との比較)
- [10.7 レコード DI の弱点](#107-レコード-di-の弱点)
- [10.8 この章のまとめ](#108-この章のまとめ)
  - [文法チェックリスト](#文法チェックリスト)
