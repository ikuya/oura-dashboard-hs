...
**戦略を書かないと GHC がどちらかを選びます。** `ToJSON` を stock 側で導出してしまうと `{"unDayText": "2024-01-01"}` のような出力になり、**JSON API のバイト互換が壊れます**。newtype に何を導出させるかは外部との契約（JSON の形、DB の値）に直結するので、明示する価値があります。
