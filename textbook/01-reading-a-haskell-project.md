...
← （なし） | [目次](README.md) | [第2章 関数・パターン・演算子](02-functions-and-patterns.md) →
...
- [1.1 このアプリは何をするか](#11-このアプリは何をするか)
- [1.2 読む順序は「依存 → 型 → 実装」](#12-読む順序は依存-型-実装)
  - [依存の向き](#依存の向き)
- [1.3 ファイル先頭の 4 ブロック](#13-ファイル先頭の-4-ブロック)
  - [文法メモ: コメントと Haddock](#文法メモ-コメントと-haddock)
  - [文法メモ: `module` 宣言と export リスト](#文法メモ-module-宣言と-export-リスト)
  - [文法メモ: `import` の 4 つの形](#文法メモ-import-の-4-つの形)
  - [文法メモ: `{-# LANGUAGE #-}` プラグマ](#文法メモ-language-プラグマ)
- [1.4 型シグネチャを読む](#14-型シグネチャを読む)
  - [GHCi で確かめる](#ghci-で確かめる)
- [1.5 レイアウト規則（オフサイドルール）](#15-レイアウト規則オフサイドルール)
  - [文法メモ: 演算子と識別子の命名規則](#文法メモ-演算子と識別子の命名規則)
- [1.6 コード生成が使われている箇所](#16-コード生成が使われている箇所)
- [1.7 この章のまとめ](#17-この章のまとめ)
  - [文法チェックリスト](#文法チェックリスト)
