...
この章で扱う 9 件のうち 8 件は `docs/code-overview.md` の「潜在的な改善点」に挙がっている**実在の問題**です（残る 1 件の 16.4 は、16.3 と同じ `AdviceJob` の設計から派生する関連問題です）。架空の練習問題ではありません。各項目を、おおむね「現状 → なぜ問題か → どう直すか → 直すべきか」の流れで講評します。
...
なお `NominalDiffTime` は ClassyPrelude が再エクスポートしていないので import の追加が要ります（`UTCTime` と `getCurrentTime` は入っています）。第 15 章の「入っているか分からなければコンパイラに聞く」の実例です。
...
修正コストはほぼゼロで、「Web から同期したのに古い欠損が埋まらない」という将来の混乱を防げます。**評価: 直すべきです。**（もし意図がそうでないなら、それはバグなので、いずれにせよ意図を明示する必要があります。）
...
最後に、単独の節を立てるほどではない小さな改善を 3 つ、まとめて 1 件として扱います。
...
**test seam の廃止**（第 10 章の `appOuraClientOverride`）が 3 つ目です。`appOuraClient :: OuraClient` を常に持つ設計にすると、`OURA_TOKEN` 未設定時の挙動が「同期時に 500」から「起動時に決まる」へ変わります。設定不備は起動時に落とす方が良い（第 7 章）という原則からは、こちらが改善です。
