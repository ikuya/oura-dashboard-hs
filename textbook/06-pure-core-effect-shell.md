...
と同じです。**`<-` は「代入」ではありません。** `m >>= \x -> ...` のラムダの引数 `x` に名前を付ける記法であり、`m` の型が `Handler Bool` なら `x` の型は `Bool` になる、という「1 段はがす」操作です。
...
この関数は効果に満ちています（HTTP 取得、DB 書き込み、ログ）。しかし**判断の中身をほとんど持っていません**。「スコアはどのフィールドか」「レコードは何日のものか」「どの API で取るか」といった判断は、すべて純粋関数に切り出されています。
...
- 現在時刻
- 乱数（`Advice.createAdviceJob` の UUID 生成は `IO` に残っていますが、ID の具体値に依存するロジックはなく、テストは `TVar` に任意の ID でジョブを直接注入して迂回できるので許容範囲。第 14 章）
